...
export default function BlockDetails() {
  ...
  const categoryLabel =
    blockCategories.find((c) => c.id === block.category)?.label ||
    block.category
      ...
      <section className="container-wide grid grid-cols-1 gap-12 py-20 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-10 lg:col-span-2">
          ...
          <div>
            ...
            <p className="mt-3 text-base leading-relaxed text-slate-450">
              {block.overview}
            </p>
          ...
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2">
            ...
            <div>
              ...
              <ul className="mt-3 flex flex-col gap-2">
                {block.facilities.map((facility) => (
                  <li
                    key={facility}
                    className="flex items-center gap-2 text-sm text-slate-450"
                  >
                    <CheckCircle2
                      size={16}
            ...
            <div>
              ...
              <ul className="mt-3 flex flex-col gap-2">
                {block.nearbyLocations.map((location) => (
                  <li
                    key={location}
                    className="flex items-center gap-2 text-sm text-slate-450"
                  >
          ...
          {availableProperties.length > 0 && (
            <div>
              ...
              <h3 className="font-display text-lg text-ink-950">
                Available Properties in {block.name}
              </h3>
        ...
        <aside className="flex flex-col gap-6">
          ...
          <div className="sticky top-24 rounded-md bg-ink-950 p-6 text-paper">
            ...
            <h3 className="font-display text-lg">
              Interested in {block.name}?
            </h3>

            <p className="mt-2 text-sm text-paper/65">
              Talk to our team about plot availability, pricing and
              payment plans.
            </p>

            <Button
              href={getWhatsAppLink('block', block.name)}
